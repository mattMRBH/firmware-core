...
TEST_CASE("FETCH parses supported root scalars and ignores cloud policy fields",
  ...
  CloudFixture f;
  const char body[] =
      R"({"pmStandard":"us-aqi","temperatureUnit":"f","measurementInterval":3600,"gpsMode":"always","gpsInterval":60,"frontLedBrightness":0,"backLedBrightness":3,"touchLedIntensity":2,"buzzerEnabled":true,"co2CalibrationRequested":true,"ledTestRequested":true,"disableCloudConnection":true,"configurationControl":"local","corrections":[]})";
  cloud_spy::fetch_body_to_write = body;
  cloud_spy::fetch_bytes_to_write = std::strlen(body);
  ...
  REQUIRE(has_go_config_field(update.update_mask, GoConfigField::GpsInterval));
  ...
  REQUIRE(update.gps_interval_seconds == GPS_INTERVAL_SECONDS_MAX);
...
TEST_CASE("FETCH rejects malformed device settings independently",
  ...
  CloudFixture f;
  const char body[] =
      R"({"temperatureUnit":"c","measurementInterval":0,"gpsMode":"ALWAYS","gpsInterval":1.5,"frontLedBrightness":4,"backLedBrightness":-1,"touchLedIntensity":3,"buzzerEnabled":"true","co2CalibrationRequested":"true","ledTestRequested":1})";
  cloud_spy::fetch_body_to_write = body;
  cloud_spy::fetch_bytes_to_write = std::strlen(body);
  ...
  REQUIRE_FALSE(has_go_config_field(update.update_mask, GoConfigField::GpsInterval));
