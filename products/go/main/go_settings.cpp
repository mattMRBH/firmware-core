...
GoSettings load_go_settings(ConfigStore &store) {
  ...
  if (store.get_int(KEY_UPDATE_INTERVAL_SECONDS, update_interval_seconds) ==
          ConfigStoreResult::OK &&
      is_update_interval_seconds_valid(update_interval_seconds)) {
...
bool GoSettings::equals(const GoSettings &other) const {
  return measure_interval_seconds == other.measure_interval_seconds &&
         update_interval_seconds == other.update_interval_seconds &&
...
bool is_go_settings_valid(const GoSettings &settings) {
  ...
  if (!is_update_interval_seconds_valid(settings.update_interval_seconds)) {
    return false;
  }
...
void print_settings(const GoSettings &settings) {
  AG_LOGI(TAG,
          "** settings | meas_int=%d update_int=%d | gps_mode=%d "
          ...
          "onboarding_done=%s **",
          settings.measure_interval_seconds, settings.update_interval_seconds, settings.gps_mode,
          settings.operating_mode, settings.auto_lock_seconds,
          settings.use_fahrenheit ? "true" : "false",
