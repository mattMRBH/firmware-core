...
void run_test_nand_storage() {
  ...
  if (PIN_NAND_CS == GPIO_NUM_NC || PIN_NAND_MOSI == GPIO_NUM_NC || PIN_NAND_MISO == GPIO_NUM_NC ||
      PIN_NAND_CLK == GPIO_NUM_NC) {
  ...
  const bool ok =
      (n == sizeof(TEST_PATTERN)) && (memcmp(buf, TEST_PATTERN, sizeof(TEST_PATTERN)) == 0);
  ESP_LOGI(TAG, "read-back: %s (read %u bytes)", ok ? "PASS" : "FAIL", static_cast<unsigned>(n));
